...
def obtener_excel_buffer() -> io.BytesIO | None:
    ...
    # --- LIMPIEZA DE DATOS PARA EXCEL ---
    # openpyxl no soporta datetimes con timezone. 
    # 'Fecha Registro' viene de Firestore (SERVER_TIMESTAMP) y es tz-aware.
    if 'Fecha Registro' in df_final.columns:
        df_final['Fecha Registro'] = pd.to_datetime(df_final['Fecha Registro']).dt.tz_localize(None)
